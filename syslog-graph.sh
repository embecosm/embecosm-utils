...
do
    gunzip -c $f | grep '\(Shut Down from ADSL Phy\)\|SHOWTIME' | tac >> ${tmpf1}
done

# Find the fields of interest, grepping out any data on Jan 1, which is change
# over of new router.
sed -n -e 's/\([[:alpha:]]\{3\}\) \([ [:digit:]]\{2\}\) \([[:digit:]]\{2\}\):\([[:digit:]]\{2\}\):\([[:digit:]]\{2\}\) .*Shut Down.*$/2017,\1,\2,\3,\4,\5,0,0,0,0/p' -e 's/\([[:alpha:]]\{3\}\) \([ [:digit:]]\{2\}\) \([[:digit:]]\{2\}\):\([[:digit:]]\{2\}\):\([[:digit:]]\{2\}\) .*SHOWTIME UpSpeed=\([[:digit:]]\+\) DownSpeed=\([[:digit:]]\+\) SNR=\([-[:digit:]]\+\) Atten=\([-[:digit:]]\+\).*/2017,\1,\2,\3,\4,\5,\6,\7,\8,\9/p' ${tmpf1} | sed -e 's/- /-0/' -e 's/Jan/01/' -e 's/Feb/02/' -e 's/Mar/03/' -e 's/Apr/04/' -e 's/May/05/' -e 's/Jun/06/' -e 's/Jul/07/' -e 's/Aug/08/' -e 's/Sep/09/' -e 's/Oct/10/' -e 's/Nov/11/' -e 's/Dec/12/' | grep -v '^2017-01-01' | tac > ${tmpf2}
...
do
    ...
    mon=$(echo ${line} | cut -d ',' -f 2 | sed -e 's/^0//')
    ...
    printf "%04d-%02d-%02d/%02d:%02d:%02d\t%d\t%d\t%d\t%d\n" ${year} ${mon} ${day} ${hour} \
           ${min} ${sec} ${down} ${up} ${snr} ${atten} >> ${tmpf1}
done < ${tmpf2}

cat > ${tmpf3} <<EOF
set xlabel "date"
stats 'tmpf3' using 1 name 't' nooutput
set xdata time
set timefmt "%Y-%m-%d/%H:%M:%S"
set format x "%d-%b"
set xrange ["2017-05-29/00:00:00" : "2017-06-06/00:00:00"]
